...
class CadastroFuncionario():
      ...
      "VALUES (%(nome)s, %(cpf)s, %(data_admissao)s, %(cargo)s, %(comissao)s)")
        ...
        try:
            ...
            if err.errno == mysql.connector.errorcode.ER_BAD_NULL_ERROR:
              raise NullDataError(f'Erro do tipo: {err}')
            elif err.errno == mysql.connector.errorcode.ER_DUP_ENTRY:
              raise DuplicateEntryError(f'Erro do tipo: {err}')
    ...
    def excluir_por_matricula(self, matricula):
        ...
        query = f"SELECT EXISTS(SELECT * from funcionarios f WHERE matricula={matricula})"
